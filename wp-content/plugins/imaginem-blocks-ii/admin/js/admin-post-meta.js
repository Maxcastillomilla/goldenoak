jQuery(document).ready(function($) {
  ...
  jQuery( '.metabox_google_font_select' ).change(function(){
	...
	var frame,
	    ...
	    images = $('#_mtheme_image_ids').val(),
	    metapageid = $('#_mtheme_image_ids').data('pageid'),
	    metanonce = $('#_mtheme_image_ids').data('nonce'),
		selection = loadImages(images);
		console.log( selection,images,metanonce,metapageid );
	...
	$('#mtheme_images_upload').on('click', function(e) {
		...
		frame.state().get('library').on( 'remove', function() {
			...
			if(models.length == 0){
			    selection = false;
				$.post(ajaxurl, { ids: '', action: 'themecore_save_images', post_id: metapageid, nonce: metanonce });
			}
		});
